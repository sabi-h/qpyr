from typing import List, Sequence, Dict, Tuple


# fmt: off
ecl_binary_indicator_map = {"L": 1, "M": 0, "Q": 3, "H": 2}

ALIGNMENT_PATTERNS_COORDS: Dict[int, List] = {
    # version: [number_of_alignment_patterns, row_col_center_coords]
    1: [0, ()],
    2: [1, (6, 18)],
    3: [1, (6, 22)],
    4: [1, (6, 26)],
    5: [1, (6, 30)],
    6: [1, (6, 34)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
    7: [6, (6, 22, 38)],
}

VERSION_CAPACITIES_BY_ECC_MAPPING: Dict[str, Dict[int, Tuple]] = {
    "L": {
        # verion: (total capacity, data capacity, error correction capacity)
        1: (26, 19, 7),
        2: (44, 34, 10),
        3: (70, 55, 15),
        4: (100, 80, 20),
        5: (134, 108, 26),
    },
    "M": {
        1: (26, 16, 10),
    },
    "Q": {
        1: (26, 13, 13),
    },
    "H": {
        1: (26, 9, 17),
    },
}

_ECC_CODEWORDS_PER_BLOCK: Dict[str, Sequence[int]] = {
    # index 0 is for padding, and is set to an illegal value
    "L": (-1, 7, 10, 15, 20, 26, 18, 20, 24, 30, 18, 20, 24, 26, 30, 22, 24, 28, 30, 28, 28, 28, 28, 30, 30, 26, 28, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30),
    "M": (-1, 10, 16, 26, 18, 24, 16, 18, 22, 22, 26, 30, 22, 22, 24, 24, 28, 28, 26, 26, 26, 26, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28, 28),
    "Q": (-1, 13, 22, 18, 26, 18, 24, 18, 22, 20, 24, 28, 26, 24, 20, 30, 24, 28, 28, 26, 30, 28, 30, 30, 30, 30, 28, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30),
    "H": (-1, 17, 28, 22, 16, 22, 28, 26, 26, 24, 28, 24, 28, 22, 24, 24, 30, 28, 28, 26, 28, 30, 24, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30),
}


_NUM_ERROR_CORRECTION_BLOCKS: Dict[str, Sequence[int]] = {
    # index 0 is for padding, and is set to an illegal value 
    "L": (-1, 1, 1, 1, 1, 1, 2, 2, 2, 2, 4, 4, 4, 4, 4, 6, 6, 6, 6, 7, 8, 8, 9, 9, 10, 12, 12, 12, 13, 14, 15, 16, 17, 18, 19, 19, 20, 21, 22, 24, 25),
    "M": (-1, 1, 1, 1, 2, 2, 4, 4, 4, 5, 5, 5, 8, 9, 9, 10, 10, 11, 13, 14, 16, 17, 17, 18, 20, 21, 23, 25, 26, 28, 29, 31, 33, 35, 37, 38, 40, 43, 45, 47, 49),
    "Q": (-1, 1, 1, 2, 2, 4, 4, 6, 6, 8, 8, 8, 10, 12, 16, 12, 17, 16, 18, 21, 20, 23, 23, 25, 27, 29, 34, 34, 35, 38, 40, 43, 45, 48, 51, 53, 56, 59, 62, 65, 68),
    "H": (-1, 1, 1, 2, 4, 4, 4, 5, 6, 8, 8, 11, 11, 16, 16, 18, 16, 19, 21, 25, 25, 25, 34, 30, 32, 35, 37, 40, 42, 45, 48, 51, 54, 57, 60, 63, 66, 70, 74, 77, 81),
}
